# To jest komórka łącząca model 3D i wykres 2D tak żeby korzystały z tych samych funkcji, 
# ogólnie w przyszłości trzeba to rozdzielić na sensowne pliki, zamiast jupytera
...
def build_grid(geo_model):
    ...
    grid["scalar"] = ra.scalar_field_matrix[0].reshape((nx, ny, nz), order="F").flatten(order="F")
    return grid
...
def build_curtain(grid, polyline, colors_rgb):
    ...
    lith_ids = np.clip(curtain["lithology"].astype(int) - 1, 0, len(colors_rgb) - 1)
    curtain.point_data["rgb"] = colors_rgb[lith_ids]
    return curtain
...
def get_boreholes_info(df_przekroj, polyline):
    """Zbiera informacje o otworach wzdłuż linii przekroju."""
    otwory, counter = [], 1
    for i, row in enumerate(df_przekroj.itertuples()):
        ...
        if not row.nazwa_otworu:
            counter += 1
        otwory.append({"nazwa": nazwa,"s": s,"z_top": row.z,"z_bottom": row.z - row.glebokosc})
    return otwory

def add_borehole_2d(plotter, otw, offset=0.5):
    """Rysuje słupek otworu i etykietę na wykresie 2D."""
    s, z_top, z_bot = otw["s"], otw["z_top"], otw["z_bottom"]
    for ds in [-offset / 2, offset / 2]:
        line_pts = np.array([[s + ds, 0, z_top], [s + ds, 0, z_bot]])
        plotter.add_mesh(
            pv.PolyData(line_pts, lines=np.array([2, 0, 1], dtype=np.int64)),
            color="black", line_width=2
        )
    plotter.add_point_labels(
        np.array([[s, 0, z_top]]), [otw["nazwa"]],
        font_size=8, text_color="black", bold=False,
        show_points=False, always_visible=True,
    )

# FUNKCJE POMOCNICZE 3D
...
def plot_przekroj_2D(geo_model, df_przekroj, z_default=240):
    ...
    otwory = get_boreholes_info(df_przekroj, polyline)
...
def plot_model_3d(geo_model, df_przekroje_z_otworami, df_boreholes, nazwa_przekroju=None, z_default=240, scale_xy=10, scale_z=20):
    ...
    if curtain_3d is not None:
        ...
        actor = pl.add_mesh(grid_exag, scalars="rgb", rgb=True, show_edges=False)
...
# Testowanie nowych przekrojów 2D z liniami rodzielajacymi warstwy
